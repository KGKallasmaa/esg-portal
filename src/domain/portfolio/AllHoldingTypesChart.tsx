...
const AllHoldingTypeLineChart = ({
  ...
  const { labels, datasets } = buildData(portfolio?.periodic || [])
...
function buildData(valueComponents: Portfolio.ValueComponents[]) {
  let data: number[] = []
  ...
  for (let index = 0; index < valueComponents.length; index++) {
    ...
    if (element) {
      data.push(element.totals.totalValue.amount)
    }
  }
...
export default AllHoldingTypeLineChart
