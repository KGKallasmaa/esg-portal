...
export default function NewIncomesButtonGroup({
  planId,
  onSuccess,
}: {
  planId: string
  ...
  return (
    <div className="flex w-full rounded-md shadow-sm" role="group">
      ...
      <button
        type="button"
        onClick={() => setShowRecurringIncomeModal(true)}
...
function NewOneTimeIncomeForm({
  ...
  const newOneTimeIncomeMutation = useNewOneTimeIncome({
    onSuccess: () => {
      onSuccess()
    },
  })
  ...
  const onSubmit = (data) => {
    ...
    newOneTimeIncomeMutation.mutate({
      planId: planId,
      title: title,
...
function NewReccuringIncomeForm({
  planId,
  onSuccess,
}: {
  planId: string
  ...
  const newRecurringIncomeMutation = useNewRecurringIncome({
    onSuccess: () => {
      onSuccess()
    },
  })
  ...
  const onSubmit = (data) => {
    ...
    newRecurringIncomeMutation.mutate({
      planId: planId,
      title: title,
      description: description,
      category: category,
